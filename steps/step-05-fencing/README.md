...
> 🟡 **여기부터 "심화"다.** step-01~04(핵심)와 달리, 05~07 은 특수 상황(비동기 + 사용자 끼어듦 + 늦은/유실 결과 + crash 복구가 동시에)에서만 필요하다. 처음 보는 거라면 "이런 문제와 해법이 있다" 정도만 잡고 넘어가도 충분하다 — 완전히 체화 못 해도 괜찮다. 복잡하게 느껴지면 그건 코드가 아니라 **문제 자체가 복잡한 것**이다.
